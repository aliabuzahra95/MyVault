...
@Composable
private fun FrozenSelectionRow(label: String, selected: Boolean, onClick: () -> Unit) {
    val colors = VaultThemeTokens.colors
    Surface(Modifier.fillMaxWidth().clickable(onClick = onClick), color = if (selected) colors.inset else colors.surface, shape = VaultShapes.md, border = BorderStroke(1.dp, if (selected) colors.accentBorder else colors.border)) {
        Row(Modifier.heightIn(min = 46.dp).padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
            Text(label, Modifier.weight(1f), style = FrozenSettingsTypography.rowTitle, color = colors.text)
            if (selected) Icon(Icons.Rounded.Verified, null, modifier = Modifier.size(16.dp), tint = colors.accent)
        }
    }
}
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(VaultSpacing.md), verticalArrangement = Arrangement.spacedBy(VaultSpacing.md)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(VaultSpacing.xs)) {
                ...
                ThemePreviewCard("System", "auto", selected = preferences.theme == VaultThemeMode.FollowSystemDark, onClick = { onThemeSelected(VaultThemeMode.FollowSystemDark) })
            }
            Row(horizontalArrangement = Arrangement.spacedBy(VaultSpacing.xs)) {
