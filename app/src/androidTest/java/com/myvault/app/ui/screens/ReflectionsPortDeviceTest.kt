...
class ReflectionsPortDeviceTest {
    ...
    private fun screenshot(name: String) {
        ...
        val directory = File(instrumentation.targetContext.getExternalFilesDir(null), "reflections-port").apply { mkdirs() }
    ...
    private val fixtures = listOf(
        item("anfal", 8, 53, "Allah does not change the favour bestowed upon a people until they change what is within themselves. A reminder to examine my own gratitude and conduct before looking outward.", 60),
        item("fatiha", 1, 1, "Beginning in the name of Allah is more than an opening phrase. I want it to shape the intention behind what follows, especially the small tasks I usually do without thought.", 50),
        ...
        item("hajj11", 22, 11, "When good touches him, he is content; when tested, he turns away. This makes me think about the difference between faith rooted in conviction and faith made conditional on ease.", 30),
        item("arabic", 2, 183, "الصيام يعلّم القلب مراقبة الله في السر قبل العلن. أريد أن أتذكر أن المقصود ليس الجوع وحده، بل التقوى ومحاسبة النفس في الكلام والعمل.", 20),
        item("mixed", 2, 185, "هدى للناس — guidance should reach my daily choices. Set aside time after reading to write one practical change, rather than measuring the day only by how many pages I read.", 10),
    )
    ...
    @Test fun compactListThemesSearchFilterSortAndLivePresentationUpdates() {
        var mode by mutableStateOf(VaultThemeMode.Dark)
        ...
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            scenario.onActivity { activity -> activity.setContent { VaultTheme(mode) {
                ReflectionsScreen(data, {}, { quranOpened = true }, { opened = it }, Modifier.statusBarsPadding())
            } } }
            text("Reflections")
            screenshot("01-dark")
            scenario.onActivity { mode = VaultThemeMode.Oled }; screenshot("02-oled")
            scenario.onActivity { mode = VaultThemeMode.Light }; screenshot("03-light")
            scenario.onActivity { mode = VaultThemeMode.Dark }
            search("Anfal"); text(fixtures[0].reflectionBody); screenshot("04-search")
            assertFalse(nodes().any { it.text?.toString() == fixtures[1].reflectionBody })
            ...
            tap(awaitNode { it.contentDescription?.toString() == "Surah: All" })
            for (attempt in 0 until 25) {
                if (nodes().any { it.text?.toString() == "22. Al-Hajj" && it.isVisibleToUser }) break
                val current = nodes()
                assertTrue("Cannot scroll Surah menu: ${current.mapNotNull { it.text }}",
                    current.firstOrNull { it.isScrollable }?.performAction(AccessibilityNodeInfo.ACTION_SCROLL_FORWARD) == true)
                SystemClock.sleep(600)
            }
            tap(text("22. Al-Hajj")); text(fixtures[2].reflectionBody); screenshot("05-surah-filter")
            search("nothing-matches"); text("No reflections found"); screenshot("06-empty-results")
            tap(awaitNode { it.contentDescription?.toString() == "Clear search" })
            text(fixtures[2].reflectionBody)
            assertFalse(nodes().any { it.text?.toString() == fixtures[0].reflectionBody })
            tap(awaitNode { it.contentDescription?.toString() == "Sort: Newest" }); tap(text("Oldest"))
            text(fixtures[3].reflectionBody)
            val bodies = nodes().mapNotNull { it.text?.toString() }.filter { it in fixtures.map { f -> f.reflectionBody } }
            assertEquals(fixtures[3].reflectionBody, bodies.first())
            tap(text(fixtures[3].reflectionBody)); assertEquals("hajj11", opened?.noteId); assertEquals("22:11", opened?.verseKey)
            tap(awaitNode { it.contentDescription?.toString() == "Sort: Oldest" }); tap(text("Qur'an order"))
            ...
            scenario.onActivity { data = emptyList(); mode = VaultThemeMode.Light }; text("No reflections yet"); screenshot("07-no-reflections")
